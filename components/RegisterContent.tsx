...
import { REGISTRATION_CATEGORIES, PASS_TIERS, WHATSAPP_NUMBER, GOOGLE_SCRIPT_URL } from '../constants';
    ...
    try {
        ...
        if (GOOGLE_SCRIPT_URL && GOOGLE_SCRIPT_URL !== "YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL") {
            await fetch(GOOGLE_SCRIPT_URL, {
                method: 'POST',
                body: JSON.stringify({ ...formData, category: categoryName, pass: passName, subject: 'Delegate Waitlist' }),
                mode: 'no-cors',
        ...
        const message = `*Priority Waitlist Request* %0A%0A*Category:* ${categoryName}%0A*Pass Type:* ${passName}%0A*Name:* ${formData.name}%0A*Phone:* ${formData.phone}%0A*Email:* ${formData.email}%0A*Organization:* ${formData.organization}%0A%0AI would like to secure my spot on the waitlist.`;
           ...
           <SectionWrapper>
              <div className="text-center mb-16">
                  <span className="inline-flex items-center gap-2 px-3 py-1 mb-4 rounded-full border border-amber-300/50 shadow-md bg-[linear-gradient(110deg,#fef3c7,45%,#ffffff,55%,#fef3c7)] bg-[length:200%_100%] animate-shimmer overflow-hidden">
                      ...
                      <span className="text-xs font-extrabold text-amber-800 uppercase tracking-widest">Opening Soon</span>
                  </span>
                ...
                {selectedCategory && (
                    ...
                    >
                         <div className="text-center mb-12 pt-8 border-t border-slate-200">
                            ...
                            <p className="text-slate-500">Unlock premiums features and exclusive access with our tiered passes.</p>
                         </div>

                         <div className="grid md:grid-cols-3 gap-8">
                            {PASS_TIERS.map((pass, idx) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className={`
                                            relative cursor-pointer rounded-3xl p-8 border-2 transition-colors duration-300 flex flex-col h-full bg-white
