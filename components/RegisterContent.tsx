...
import { REGISTRATION_CATEGORIES, PASS_TIERS, PAYMENT_PORTAL_URL, GOOGLE_SCRIPT_URL, COMPETITIONS_LIST } from '../constants';
...
const RegisterContent: React.FC = () => {
  ...
  const handleCompetitionToggle = (title: string) => {
  ...
  const handleInitialSubmit = (e: React.FormEvent) => {
    ...
    // If student, show competition modal first
        ...
        if (GOOGLE_SCRIPT_URL && GOOGLE_SCRIPT_URL !== "YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL") {
            await fetch(GOOGLE_SCRIPT_URL, {
                ...
                body: JSON.stringify({ 
                    ...
                    competitions: selectedCompetitions.join(', ') || 'None'
                }),
                mode: 'no-cors',
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
                ...
                >
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div>
                            <h3 className="text-xl font-bold text-slate-900">Select Competitions</h3>
                            <p className="text-sm text-slate-500">Interested in participating? Select below (Optional)</p>
                        </div>
                        <button onClick={() => setShowCompetitionModal(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
                    ...
                    <div className="p-6 overflow-y-auto custom-scrollbar">
                        <div className="grid gap-4">
                            {COMPETITIONS_LIST.map((comp, idx) => {
                                const isSelected = selectedCompetitions.includes(comp.title);
                                return (
                                    <div 
                                        key={idx}
                                        onClick={() => handleCompetitionToggle(comp.title)}
                                        className={`
                    ...
                    <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                        ...
                        <button 
                            ...
                            className="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-500 transition-colors shadow-lg shadow-blue-500/25 flex items-center gap-2"
                        >
                            {isSubmitting ? <Loader2 className="animate-spin" /> : <>Confirm & Pay <ChevronRight size={18} /></>}
                        </button>
                    </div>
                </motion.div>
            </div>
